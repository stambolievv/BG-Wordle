...
export class Storage {
   ...
   */
  static getScore(wordLength) {
    return Number(sessionStorage.getItem(`${KEYS.score}-${wordLength}`)) || 0;
  }
   ...
   */
  static setScore(wordLength, value) {
    sessionStorage.setItem(`${KEYS.score}-${wordLength}`, String(value));
  }
   ...
   */
  static getHighscore(wordLength) {
    return Number(localStorage.getItem(`${KEYS.highscore}-${wordLength}`)) || 0;
  }
   ...
   */
  static setHighscore(wordLength, value) {
    localStorage.setItem(`${KEYS.highscore}-${wordLength}`, String(value));
  }
   ...
   */
  static setHardMode(value) {
    localStorage.setItem(KEYS.hardMode, String(value));
  }

  /**
   * @description The stored theme preference. Falls back to `prefers-color-scheme` when
  ...
  /**
   * @description Whether the user has ever explicitly set a theme preference.
   * `false` means the active theme is purely OS-derived and should follow system changes.
   * @returns {boolean}
   */
  static hasThemePreference() {
   ...
   */
  static getSwapButtons() {
    return localStorage.getItem(KEYS.swapButtons) === 'true';
  }
   ...
   */
  static setSwapButtons(value) {
    localStorage.setItem(KEYS.swapButtons, String(value));
  }
  ...
  static getWordLength() {
    return Number(localStorage.getItem(KEYS.wordLength)) || Config.defaultWordLength;
  }
   ...
   */
  static setWordLength(value) {
    localStorage.setItem(KEYS.wordLength, String(value));
  }
